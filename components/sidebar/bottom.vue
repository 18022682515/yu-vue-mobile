...
export default {
	...
	computed:{
		style(){
			return { height:this.size+'vh',transform:`translateY(${this.bottom}px)`,top:100-this.size+'vh' };
		}
	},
    watch:{
        value(n,o){
    ...
    methods:{
        ...
        ani(bool){
            if(bool){
                animate(this.element,[{ transform:{ translateY:0 } },{ duration:300 }]).then(()=>{
                    this.bottom = 0;
                });
            }else{
                animate(this.element,[{ transform:{ translateY:this.height } },{ duration:300 }]).then(()=>{
